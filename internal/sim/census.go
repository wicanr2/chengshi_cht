...
func (w *World) SetValves() {
	...
	// ⚠ 住宅人口在這裡除以 8：Map 上的住宅人口單位是「每格 8 人」。
	normResPop := float64(w.ResPop) / 8
	w.LastTotalPop = w.TotalPop
	w.TotalPop = int(normResPop) + w.ComPop + w.IndPop

	employment := 1.0
	if normResPop != 0 {
		employment = float64(int(w.ComHis[1])+int(w.IndHis[1])) / normResPop
	}
	...
	laborBase := 1.0
	if t := float64(int(w.ComHis[1]) + int(w.IndHis[1])); t != 0 {
		laborBase = float64(w.ResHis[1]) / t
	}
	if laborBase > 1.3 {
		laborBase = 1.3
	}
	...
	intMarket := (normResPop + float64(w.ComPop) + float64(w.IndPop)) / 3.7
	...
	pjIndPop := float64(w.IndPop) * laborBase * extMarket
	if pjIndPop < 5 {
		pjIndPop = 5
	}
	...
	cratio := pjComPop
	if w.ComPop != 0 {
		cratio = pjComPop / float64(w.ComPop)
	}
	iratio := pjIndPop
	if w.IndPop != 0 {
		iratio = pjIndPop / float64(w.IndPop)
	}
	if rratio > 2 {
		rratio = 2
	}
	...
	rr := int((rratio-1)*600) + taxTable[z]
	cr := int((cratio-1)*600) + taxTable[z]
	ir := int((iratio-1)*600) + taxTable[z]
