...
func (p *Player) year() {
	...
	// （城市萎縮），所以取中間的 6%。
	//
	// ⚠ 2026-08-30 重掃過一次（達斯維利五顆種子平均人口，配上暫時放寬的
	// `extendRoad` 門檻）：8% 是 31 812、6% 是 25 160、10% 只剩 9 273。
	// **還是沒改**——同一組設定讓舊金山從 93 640 崩到 37 440、資金歸零。
	// 舊金山是災後五年的復原劇本，加稅加擴張正好相反。
	// 分不開它們的不是「目標」，是時間尺度與起始狀態。見 WORKLOG 同日。
	//
