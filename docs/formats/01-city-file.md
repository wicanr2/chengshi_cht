...
| 位移 | 長度 | 內容 |
|---:|---:|---|
| 0 | 128 | 檔頭：`00 0d`、城市名、零填、魔數 `CITYMCRP` |
| 128 | 2880 | 六個歷史陣列，與上表相同 |
| 3008 | **256** | `MiscHis` —— **128 個 short**，不是 Micropolis 的 120 |
| 3264 | 24000 | 地圖 120×100 |

檔案在 27248 結束，**比 3264+24000 少 16**：地圖最後 8 格（x=119、y=92–99）
沒有寫進去。原因未解；讀的時候補零。

⚠ **不要用檔案大小去推檔頭長度。** 27248 − 27120 = 128 會讓人以為「檔頭 128、
其餘照舊」，而那個答案**看起來完全正常**：純量全部讀得對（CityTime 3、
資金 5000、稅率 7），地物的格數分布也對——因為整張地圖只是**平移了 8 格**，
counts 不變。只有逐格比才看得出來：那樣讀 12000 格裡只有 3191 格對得上，
正確讀法有 11923 格（其餘是存檔那一刻模擬相位不同造成的差異，
見 [`../re/18-dos-parity.md`](../re/18-dos-parity.md)）。

三種來源的判別與補零都在 `internal/game/save.go` 的 `normalizeCityBytes`。
