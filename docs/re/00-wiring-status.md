...
| [`docs/re/20-terrain-editor.md`](20-terrain-editor.md) | 地形編輯器：主畫面版面、六格工具盤、四個畫筆與工具描述表、油漆桶、五千格復原環＋四份快照、三個選單的十七條命令、清除人造物、平滑河流的前置、Game Year、參數對話框 | **已接** | 規則層在 `internal/sim/editor.go`（畫筆寫的 16 位元字、`FillAt` 的三個帶、`Undo` 的環與快照、`ClearUnnatural`、`ResetRiverEdges`）與 `internal/sim/terrain.go` 的 `TerrainParams.EditorDOS`（樹叢數量 `3 × pct`）；呈現層在 `internal/ui/terrain_screen.go`（三個選單、工具盤命中、快速鍵、年份輸入）、`internal/ui/terrain_draw.go`（版面與六格工具盤）與 `internal/ui/terrain_editor.go`（參數對話框）。未接的只剩「確認框的版面」與「進度框與年份框的絕對位置」——兩者未解，見該筆記 §十六 |
