...
| [`docs/re/20-terrain-editor.md`](20-terrain-editor.md) | 地形編輯器：招牌畫面、`TERRAIN.CFG`、圖形檔沒有介面美術、LZEXE 0.91 解包、對話框版面反組譯 | 未接 | 版面已收攏成 [`docs/spec/terrain-editor.md`](../spec/terrain-editor.md)（**READY**），但 remake 還沒實作。規則層早就接好了（`internal/sim/terrain.go` 的 `GenerateMap`＋`TerrainParams`＋`SmoothTerrain`）；缺口只剩「百分比 → `TerrainParams` 數值域」的換算式 |
