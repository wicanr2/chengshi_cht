...
| [`docs/re/20-terrain-editor.md`](20-terrain-editor.md) | 地形編輯器：招牌畫面、`TERRAIN.CFG`、圖形檔沒有介面美術、`TERRAIN.EXE` 是 LZEXE 0.91 打包 | 未接 | 介面還沒看到（DOSBox-X 上跑不起來，INT 6），所以沒有可實作的規格。規則層要用的東西早就接好了（`internal/sim/terrain.go` 的 `SmoothTerrain`、`internal/game/save.go`），缺的只有版面 |
