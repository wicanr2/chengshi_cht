...
| 波士頓 | 5282 | 110 | **`Feb 1959`** | 1959 | 2010 | 2010 |

（東京那一列是先前記錄的觀察，其餘四列是實測。波士頓那一列是驗第 9 庫時
順手截到的，見 [`../formats/03-pgf-graphics.md`](../formats/03-pgf-graphics.md) §十一。
月份也對得上：`CityTime mod 48 = 2` → `Feb`，與達斯維利同。）
...
- **`DS` 的實際值找到了兩個**（掃 `mov r16,imm16` ＋ `mov ds,r16` 的所有形式）：

  | 段值 | 出現次數 | 是什麼 |
  |---|---:|---|
  | `33AEh` | `ds` 6 ＋ `es` 16 | **主資料段（DGROUP）**，字串應該在這裡 |
  | `24ABh` | `ds` 27 ＋ `es` 44 | 音效卡驅動自己的段（§五之五 已經記過）|

- **但映像不是從段 0 起算**，所以 `33AEh` 換不成檔案位移：`0x33AE × 16` 已經
  超過映像大小（175 011 位元組）。試著反推映像的基底段——掃 `S = 0x1000…0x3400`，
  看哪個 `S` 能讓多個字串同時被 `push imm16`／`mov ax,imm16` 參照——
  **四個字串裡最多只對上兩個，而且各只有一次命中**，那是 175 KB 裡三位元組
  樣式的雜訊水準，不是訊號。所以**這一版的字串參照不走立即值**
  （可能是 `lea`、可能經過索引暫存器），光靠樣式搜尋找不到。

- **下一個入口**：不要再用樣式搜尋。正路是把載入映像**重建成有分段的資料庫**
  ——拿 `tools/unpack_simcity_exe.py` 解出來的重定位資訊（或原始 `.EXE` 的
  MZ 重定位表）在 IDA 裡建段，`DS = 33AEh` 已知，字串 xref 一通，
  直接看誰把參數推給 `%3s %4d` 就結案。
...
**第三份副本佐證同一個結論**（2026-09-02）。軟體世界 1990 年重新打包的 Maxis
地形編輯器磁片（`E220-1.ZIP`，玩家自備）裡也有一份 `SOUNDDAT.PSF`，
檔案時間戳 1989-10-16，與 `DATA/SOUNDDAT.PSF` **逐位元相同**
（SHA-256 `10572c0b3de9199bb3ff966f5779c1e036c62063bb15af88559cee91ab84d556`），
與根目錄那一份不同（`77cf45a1…`）。三個獨立來源裡有兩個站在同一邊：
1989 年的地形編輯器、1991 年的 `.V4`，對上一份後來的重打包。

⚠ **這條證據回答的是「哪一份是原廠內容」，不是「遊戲執行時開哪一份」。**
後者仍然未解，而且不會由再多的副本比對來回答——那要讀執行檔裡的檔名字串。
...
### 預設值那一段的版面（2026-08-30 補）

把那一段位元組原樣看一次（映像 `0x24cba` 起）：

```
message.ptf\0 sounddat.psf\0 monodat.pgf\0 Classic\0 Loading %s graphics\0 DATA\0 DATA\0 \0
```

**三個檔名、三個目錄，最後一個是空字串。** 三個檔名正好是 `.PGF` 檔頭的
三個欄位（`MsgFile`／`SoundFile`／`MonoFile`，`internal/assets/pgf.go`），
順序一致；三個目錄一一對應就是 `DATA\`、`DATA\`、空。這與兩件實測對得上：
訊息檔那一欄的實測路徑是 `C:\DATA\west_msg.ptf`，而印表用的 `monodat.pgf`
的錯誤訊息旁邊放的是**模式目錄名** `mono` 不是 `DATA`。

所以 remake 取 `DATA/` 那一份（`internal/ui/sound.go` 的 `soundFile`）的依據
是「三欄位一一對應 ＋ 姊妹欄位實測」，**不是字串剛好相鄰**。

### 剩下的路：三個都試過，DOSBox 這條走不通

`sounddat.psf` **只在用基本圖形集時才會被讀**（風格檔的檔頭都指名自己的
`*_SND.PSF`，而那些檔案只存在於 `DATA/`）。想直接觀測「它讀的是哪一份」，
2026-08-30 用基本圖形集試了三種弄壞法，**三種都沒有任何反應**：

| 做法 | 開機畫面 |
|---|---|
| 拿掉 `DATA/SOUNDDAT.PSF` | 照常開起來 |
| 連根目錄那份與 `SOUNDDAT.V4` 一起拿掉 | 照常開起來，**逐像素相同** |
| 把 `DATA/SOUNDDAT.PSF` 換成 12 個位元組的垃圾 | 照常開起來，**逐像素相同** |

訊息檔缺檔會當場中止並印出完整路徑，音效檔缺檔什麼都不會發生——
**音效檔不是開機時載入的**，要等第一次發聲才讀，而 DOSBox 這套環境
放不出這張卡的聲音（§五之一）。所以這條路到此為止。

等級維持**強證據**。要定案只剩反組譯開檔那一段，或找出那個除錯開關
讓 `\nFound file %s` 印出來。

## 九、「弄壞資料檔再比截圖」的正確做法（2026-08-30 補）

§八 用這個方法排除了一條路，當時的訊號夠大（兩萬像素）所以結論站得住。
拿它去問更細的問題（某一個圖形庫有沒有被畫出來，訊號可能只有兩百多像素）
就會踩到四個坑，全部實際踩過一次。

### 坑一：`.PGF` 是壓縮的，但不必寫壓縮器

DOS 版所有資料檔共用一種 LZSS（[`../formats/02-dos-lzss.md`](../formats/02-dos-lzss.md)），
所以不能直接改位元組。**但也不必實作壓縮搜尋**：那個格式的旗標位元是 1 就代表
「後面接一個字面位元組」，所以**每八個位元組前面加一個 `0xFF`** 就是合法輸出。
檔案大 12.5%，原版解碼器照樣讀得回來。工具是 [`../../tools/pgfpatch`](../../tools/pgfpatch)
（解壓 → 改指定庫的像素 → 全字面重壓）。
用法：`tools/go.sh run ./tools/pgfpatch <輸入.PGF> <輸出.PGF> [庫號] [填入值]`。

**重壓但不改內容的那一份要當成獨立的對照組跑**，用來證明重壓本身不會讓畫面變樣。

### 坑二：新城市的地形是隨機的

開新城市每次生成的地圖都不一樣，逐點比對整片都會亮。**一定要載劇本。**

### 坑三：不暫停的話，同檔跑兩次就差兩千多個像素

遊戲一直在跑，主機負載會讓「按下暫停之前跑了幾個模擬刻」浮動。實測同一個檔、
同一支腳本跑兩次，地圖區差 **2 347–2 482 像素**。按 `0` 暫停之後畫面就完全靜止
（同一畫面隔六秒各截一張，逐位元相同），但**暫停之前累積的差異留在城市狀態裡**，
所以噪音降不到零。
...
（960 張）塗成 0 也是**零像素差異**——看起來像乾淨的否定結論，其實是沒有訊號。
...
編輯視窗的地圖格網原點是螢幕座標 `x = 256 + 16i`、`y = 239 + 16j`，一畫面 512 格
（量法：把畫面上每個 16×16 位移都拿去比對第 0 庫的 960 張圖塊，命中數最高的位移
就是原點；調色盤要先從 `.PGF` 的 `0/80/160/240` 正規化成 DOSBox 的 `0/85/170/255`）。

在這個格網上，**504 格逐位元等於第 0 庫的某一張圖塊**，剩下 8 格是
**3×3 工具游標的外框**（中心格沒被碰到，所以剛好是環狀的八格）。

這個判準不看城市長什麼樣，只看「有沒有非圖塊的東西疊上來」，所以**對模擬進度
免疫**；而那八格游標外框本身就是內建的正對照，證明它抓得到疊圖。
腳本：[`../../tools/dosbox/act-b9.txt`](../../tools/dosbox/act-b9.txt) ＋ 掃描器（[`../../tools/shot_tilescan.py`](../../tools/shot_tilescan.py)，量法寫在
[`../formats/03-pgf-graphics.md`](../formats/03-pgf-graphics.md) §十一）。
