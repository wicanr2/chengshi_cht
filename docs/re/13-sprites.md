...
**Tokyo 劇本逐 frame 8000/8000。** 那座城市有機場、港口與鐵路，劇本災難
又是怪獸，所以八種精靈裡除了永遠不會出現的巴士全部上場（火車、船、飛機、
直昇機、怪獸、爆炸）。判準是每個 frame **場上每一隻精靈的十八個欄位**
（`sim Sprites`）加上整張地圖的雜湊，見 [`12-tick-parity.md`](12-tick-parity.md)。

追這條線修掉的錯裡有兩個值得單獨記著：

**一、`MoveObjects` 不能重建串列。** 原版走鏈結串列，`MakeNewSprite`（爆炸）
把新節點插在游標**前面**——這一輪不會被走到，但**留在串列裡**。Go 版原本
用 `kept := s.list[:0]` ＋ `s.list = kept` 重建，迴圈中途插進來的爆炸就沒了：
怪獸拆掉一棟大樓不會爆炸。現在走「這一輪開始時的快照」，回收時才從
`s.list` 就地摘掉。

**二、`SetTrafMem` 會改直昇機的目的地。** `s_traf.c:126`：車流密度超過 240
而且 `Rand(5)` 命中時，記下 `TrafMaxX/Y`，並把**自動巡邏中**（`control == -1`）
的直昇機引過去。這條路徑不抽亂數，任何以抽樣次數為判準的測試都看不見它漏掉。

**三、十八個欄位不含幾何。** 寬高、繪圖偏移、熱點只跟型別有關，`sim Sprites`
不倒它們。對拍載入時要用 `applyGeom` 補回去——少了熱點，`spriteNotInBounds`
會把 x = −47 的船（熱點 48，其實還在圖上）算成出界。
