...
![繁体字中国語のタイトル画面](docs/images/ja/title.png)
...
![ゲーム画面：デトロイト 1972](docs/images/ja/city.png)
...
![地形エディタ](docs/images/ja/terrain-editor.png)
...
![「地形」メニューを開いたところ](docs/images/ja/terrain-menu.png)

「地形」メニュー。繁体字中国語版では、この七つのコマンドの語がすべて、
軟體世界が 1990 年にこのエディタ用に出した中国語マニュアルの言い回しそのままである
——下の[中国語の語はどこから来たか](#provenance)を参照。
...
| | |
|---|---|
| ![同じ都市の中世スタイル](docs/images/ja/style-medi.png) | ![地図ウィンドウの犯罪率レイヤー](docs/images/ja/maps.png) |
...
![古代アジアのツールパレットとツール名欄](docs/images/style-tools.png)

*（この 1 枚だけ繁体字中国語版の画面である。それがこの節の論点だからだ。）*
古代アジア拡張。パレットの警察のマスを押すと、ウィンドウ左下のツール名欄に
**衙門：$500** と出る。衙門（がもん）は王朝時代の役所を指す語。同じマスは中世で 警局、
西部で 警長（保安官）、未来ヨーロッパで 警防部、月で 月球警署 になる。
これらはこのプロジェクトが考えた語ではなく、**1990 年代の台湾の代理店が
実際に印刷した語**である。

本プロジェクトは当時の中国語マニュアルを**四冊**持っており、画面上の名詞は
そのどれかの何ページ目かに必ず辿れる：

| 画面の語 | どのマニュアル | ページ |
|---|---|---|
| 悲情城市、明日之都、市鑰、八つのシナリオ都市 | 軟體世界 珍藏版 29 | p.13–14 |
| **大都會區**（`Megalopolis`）、大都會（`Metropolis`）| 軟體世界 珍藏版 29 | p.1、p.18 |
| 推土機、鐵軌、全自動整地、予算・評価ウィンドウの各欄 | 軟體世界 珍藏版 29 | p.23–58 |
| **地形編修程式**、開濶地、綠地、航道、均佈、**回手**（アンドゥ）、地形變數 | 軟體世界 220『地形編修程式』 | p.13–22 |
| **衙門**、**廟宇**、**摔角場**、**港灣**、水井灌溉系統 | 電腦休閒世界 022『古城風情系列』 | p.56 図版 |
| **警局**、**馬術競技場**、城堡／**警長**、**賽馬場**、**金礦區** | 電腦休閒世界 022 | p.59、p.62 図版 |
| **小鎮**、**首都** ほか都市クラス六段階 | 電腦休閒世界 022 | p.46 |
| **警防部**、**雷射競技場**、**水翼船港**、**太空站**、核子融爐電廠 | 電腦休閒世界 024『回到未來系列』 | p.17–18 図版 |
| **月球警署**、**自由港**、**太空梭站**、**引力波電廠** | 電腦休閒世界 024 | p.19 図版 |

**四冊は四つの独立した証拠ではない。** 電腦休閒世界 022 の第一部と軟體世界 珍藏版 29 は
**同一の中国語原稿**である——メニュー、ツール、ウィンドウの各欄、八つのシナリオ解説まで
一文ずつ一致し、図番号（「図八 編集ウィンドウ」）まで揃う。しかも二社は別々に MAXIS から
ライセンスを受けた別の代理店だ。024 の PART B はさらに 220 の再録である。
系譜図と項目ごとの判断はすべて
[`docs/manual-cht/naming-crosswalk.md`](docs/manual-cht/naming-crosswalk.md) にある。

**マニュアルの語が原文より粗い場合もマニュアルを採り、そのずれを記録する。**
`Sumo arena` は 摔角場（相撲ではなくレスリング）、`Rodeo` は 賽馬場（競馬）、
`Hovercraft Port` は 水翼船港——**水中翼船であってホバークラフトではない。
1990 年時点の誤訳**だが、当時の語としてそのまま残し、注を付けた。

逆に、意図的にマニュアルに従わなかったのが三箇所ある：月の二つの発電所（024 の図版が
本文と矛盾している）、月のスタジアム（従うと文字列がベースの語に戻ってしまい、
それはまさに `TestStyleFilesDoNotInheritBaseWording` が防いでいる見落としである）、
そして `Query`（022 は「質詢回答」＝議会の質疑と訳しており、マスを押して情報を見る
動作と合わない）。

![1/4 に縮小](docs/images/ja/zoom.png)
...
![同じ都市を六つの表示モードで](docs/images/ja/modes.png)
...
![言語設定](docs/images/ja/settings.png)

四言語とも、インターフェイスとゲーム内テキストの全体をカバーしている。ステータス
メッセージ、画像メッセージ、八つのシナリオ説明、調査パネルの程度語、予算と評価の
項目、そして**六つの拡張ディスクがそれぞれ使う言い回し**——古代アジアの発電所は
「水車」、中世の鉄道は「荷馬車道」で、翻訳も基本ファイルに戻らずそれに合わせて
変わる。これはテストで固定してある（`TestStyleFilesDoNotInheritBaseWording` と
その日本語版）。原文の語が変わったのに訳語が変わっていなければ赤くなる——この種の
抜けはほかの方法では見えない。ファイルは揃っていて、空文字もなく、画面も正常に
見えるからだ。英語はバージョン管理に入れず、実行時にプレイヤー自身の `.PTF` から
読み込む。
...
台湾で当時この作品のために印刷された中国語マニュアルを、本プロジェクトは四冊持っている。
四冊ともページごとに翻刻した：

| マニュアル | 発行 | 収録範囲 | 翻刻 |
|---|---|---|---|
| 珍藏版 29『模擬城市』| 軟體世界 | 操作手冊＋参考手冊（p.1–82）| [`docs/manual-cht/`](docs/manual-cht/) |
| 220『模擬城市 地形編修程式』| 軟體世界 | 地形エディタ（p.1–23）| [`sw220-terrain/`](docs/manual-cht/sw220-terrain/) |
| 022『模擬城市－古城風情系列』| 電腦休閒世界（MAXIS 公認）| 本編＋古代アジア／中世／西部（p.1–64）| [`chw022-ancient/`](docs/manual-cht/chw022-ancient/) |
| 024『模擬城市－回到未來系列』| 電腦休閒世界（MAXIS 公認）| 未来アメリカ／ヨーロッパ／月＋220 の再録（p.1–40）| [`chw024-future/`](docs/manual-cht/chw024-future/) |

このうち珍藏版 29 のものは、oldgame.tw の「軟體世界マニュアル補完計画」に
...
![軟體世界が 1990 年に作った台湾地図を、このリメイクで開いたところ](docs/images/ja/map-taiwan.png)

台湾本島、中央山脈、西の澎湖、南東には緑島と蘭嶼。都市名は都市ファイルの 128 バイトの
ヘッダから読んでいる。タイトルバーの `TAIWAN` はファイルにもとから書かれていた文字だ。

![軟體世界が 1990 年に作った高雄地図](docs/images/ja/map-kaohsiung.png)
...
![台北](docs/images/ja/map-taipei.png)
![台中](docs/images/ja/map-taichung.png)
![台南](docs/images/ja/map-tainan.png)
...
![古城風情の 3 スタイル](docs/images/ja/styles-ancient.png)

**Graphics Set 2 — Future Cities（回到未來）：未来アメリカ、未来ヨーロッパ、月面居住地**

![回到未來の 3 スタイル](docs/images/ja/styles-future.png)
...
![地形エディタのパラメータダイアログ](docs/images/ja/terrain-params.png)
...
| 中国語化と多言語 | 9.8 | 簡体字は繁体字の**字面**変換であって用語のローカライズではない。日本語は本プロジェクトの新訳で、突き合わせる原版が存在せず、**母語話者の校閲もまだ** |
...
**今回動いたのは 1 行だけ：中国語化と多言語 9.5 → 9.8。** その行の減点が 3 つから
2 つに減ったからだ。日本語はこれまでインターフェイスのラベル止まりで、ステータス帯・
画像メッセージ・八つのシナリオ説明は繁体字中国語に落ちていた。それが全部訳され、
六つの拡張ディスクそれぞれの言い回しも切り替わるようになり、テストで固定してある。
残る 2 つはそのままだ——簡体字は字面変換であること、日本語に母語話者の校閲がないこと。
だから 10 ではなく 9.8。

**ほかの 7 行は動いていない。今回の変更が少なくないにもかかわらず、だ。** 点数が
追っているのは「まだ解けていないもの」であって「どれだけ手を動かしたか」ではない。
表示層はこの回で二十数か所の文字列をテキスト層に通し、テストを 3 本足したが、
あの行に並ぶ 3 つの未解決（ウィンドウの既定位置、オリジナルの原子力発電所の建て方、
数値レイヤーの第 5 段階の閾値）は 1 つも減っていない。
...
| [`docs/manual-cht/`](docs/manual-cht/) | 中国語マニュアル**四冊**のページ単位の翻刻と [`naming-crosswalk.md`](docs/manual-cht/naming-crosswalk.md)（系譜・訳語対照・判断の記録）|
