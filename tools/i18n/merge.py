#!/usr/bin/env python3
"""把譯文合併進 translations/messages/*.toml。
...
用法（在 docker 裡跑，見 tools/i18n.sh）：
    tools/i18n/merge.py translations/messages
"""
import os
import re
import sys
...
def merge(path, table):
    ...
    def repl(m):
        ...
        if not zh:
            return m.group(0)
        filled += 1
        body = re.sub(r'zh = ".*?"(?=\n)', 'zh = "%s"' % toml_escape(zh), body, count=1)
        return '["%s"]\n%s' % (key, body)
...
def main():
    d = sys.argv[1] if len(sys.argv) > 1 else "translations/messages"
