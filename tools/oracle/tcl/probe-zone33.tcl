# 分岔診斷：跑到第 N 個 frame，然後把三座空住宅區的衍生陣列讀出來。
# N 由 tools/oracle/probe-zone.sh 產生時代入。
...
sim Frame 33
puts stdout "AT [sim Fcycle] [sim Scycle] [sim Valves]"
puts stdout "Z 15 18 tile [sim Tile 15 18] lv [sim Mem LandValueMem 7 9] pol [sim Mem PollutionMem 7 9] crime [sim Mem CrimeMem 7 9] pop [sim Mem PopDensity 7 9] trf [sim Mem TrfDensity 7 9] rog [sim Mem RateOGMem 1 2]"
puts stdout "Z 20 18 tile [sim Tile 20 18] lv [sim Mem LandValueMem 10 9] pol [sim Mem PollutionMem 10 9] crime [sim Mem CrimeMem 10 9] pop [sim Mem PopDensity 10 9] trf [sim Mem TrfDensity 10 9] rog [sim Mem RateOGMem 2 2]"
puts stdout "Z 25 18 tile [sim Tile 25 18] lv [sim Mem LandValueMem 12 9] pol [sim Mem PollutionMem 12 9] crime [sim Mem CrimeMem 12 9] pop [sim Mem PopDensity 12 9] trf [sim Mem TrfDensity 12 9] rog [sim Mem RateOGMem 3 2]"
set _m {} ; for {set y 0} {$y < 100} {incr y} { for {set x 0} {$x < 120} {incr x} { lappend _m [sim Tile $x $y] } } ; puts stdout "M33 [llength $_m] [join $_m ,]"
