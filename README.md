...
| 可玩完成度 | 7 | [`tools/playtest.sh`](tools/playtest.sh) 的 21 項實機判定；端到端成長測試（30 年 → 人口 7360、評分 795）；八個劇本都跑到勝敗判定 | **沒有人真的從頭玩通任何一個劇本。** 不出手跑只有 2/8 過（Bern、Boston），那證明的是判定機制會觸發，不是「玩得贏」 |
...
扣分集中在兩件事——**沒有聲音**，以及**沒有人真的把一座城市或一個劇本玩完**。
前者卡在證據（見上一節），後者是還沒做的工。
