...
| 呈現層與操作 | 9 | **版面照原版 DOS 重現**（[`docs/spec/ui-layout.md`](docs/spec/ui-layout.md)）：選單列 ＋ 四個下拉選單、編輯視窗（資金訊息帶／工具盤／地圖／工具帶／需求指標）、City Form 視窗、三個資料視窗做成浮動視窗，位置與大小都量自原版截圖；工具盤與圖層圖示直接用原版 `.PGF` 的美術；工具盤十四格的順序逐格實測確認 | 視窗只能搬不能改大小（原版 `Ctrl-R`）；查詢工具的彈出視窗還沒對拍；原版五段速度對到規則層四段 |
