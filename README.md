...
對到什麼程度：**兩份 8000 個 frame（各 500 個遊戲刻）的對拍都完全一致**
——一份是空地上的小實驗（13 582 次抽樣），一份載入 Dullsville 劇本
（**119 821 次抽樣**，有人口、交通、稅收與週期性的城市評估）。另外兩份載入東京（劇本災難是怪獸，唯一會動到精靈的），一份
跑 8000 個 frame 比抽樣次數（46/8000），一份跑 400 個 frame 但**逐 frame 比對
每一隻精靈的十八個欄位**（46/400）。精靈那一側是現在的前線。每一個 frame
抽了幾次亂數、亂數狀態、五個週期性掃描落在哪一刻、三個需求閥門、城市評估
的分數與問題表都相同，跑完之後 12 000 格地圖與資金也相同。
...
frame，拿掉之後直接跑完八千個。東京那份則逼出**載入城市時整段清場的儀式
沒做**——原版讀檔之後會把地價、汙染、犯罪、交通與精靈全部歸零，我們沒有，
所以在遊戲裡讀第二座城市會留著前一座的數字。那是逐 frame 對拍抓到的真的
遊戲 bug，不只是對拍問題。細節見
