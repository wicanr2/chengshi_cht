...
從原版的招牌開始，先在台灣地圖上蓋一座城市讓它跑二十五年，接著依序巡過台灣、
台北、台中、台南、高雄，最後在高雄再蓋一座。畫面上的圖塊來自玩家自備的原版，
影片本身是本重製版跑出來的。發行包裡的 `promo.mp4` 另有一段配樂，
那是本專案自己合成的（[`tools/promomusic`](tools/promomusic)）——原版沒有背景音樂，
四條證據見 [`docs/re/19-no-music.md`](docs/re/19-no-music.md)；上面內嵌的 GIF 沒有聲音。
