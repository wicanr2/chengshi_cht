...
對到什麼程度：**住宅、商業、工業三種分區的微實驗都逐次元完全一致**
（692.5／564.2／949.2 刻，地圖零差異）；整座城市的分段對拍 23 段裡 9 段完全一致
（含完整城市評估的段落），資金軌跡與原版相同。

工業區那個實驗原本怎麼搜都對不上，症狀看起來像成長判斷寫錯了。追下去發現
問題不在工業區，而在**起始狀態少設了一個看不見的變數**：`Scycle` 決定哪一刻
要跑犯罪／汙染／人口那幾個掃描，而它從外面觀察不到。設錯的話那幾個掃描落在
不同的刻，但抽樣次數還是對的——只有它們吃到的亂數值不一樣。細節與方法見
[`docs/re/12-tick-parity.md`](docs/re/12-tick-parity.md) §6。
...
- **逐刻對拍還沒完全收斂**：分段對拍 23 段裡 14 段仍有差異。已知的下一步是
  把 `Scycle` 用接力的方式加進搜尋（見上）。
