...
| 可玩完成度 | 8 | 上面三項，加上**與 DOS 原版的抽樣對拍**（[`docs/re/18-dos-parity.md`](docs/re/18-dos-parity.md)）與**自動玩家**（[`internal/autoplay`](internal/autoplay)）：只用玩家在畫面上做得到的操作（`ApplyTool` 與稅率），八個劇本通關五個（達斯維利、舊金山、里約沒過）；不出手只過一個 | 三個劇本還過不了，而且**還是沒有人用手玩通過**——自動玩家證明的是「規則允許贏」，不是「介面好用」。底特律那一關是靠城市萎縮達成犯罪門檻，稱不上玩得漂亮 |
