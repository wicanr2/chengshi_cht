...
| 音效 | 3 | 容器格式解開（九份檔各八段 4 位元 PCM）；執行檔脫了殼，符號表解出來了——音效的三支常式都在模組 `0x23`（[`docs/re/16-dos-oracle.md`](docs/re/16-dos-oracle.md) §5）| **還是沒有接進遊戲。** 那八段只走外接 DAC，手上兩條 DAC 路都走不通；而模組編號還沒對應到脫殼後映像的位址，所以讀不到發聲常式，「哪一段是哪個事件」仍未解 |
