...
對到什麼程度：**8000 個 frame（500 個遊戲刻、13 582 次抽樣）逐 frame
完全一致**——每一個 frame 抽了幾次亂數、五個週期性掃描落在哪一刻、三個需求
閥門的值都相同，跑完之後 12 000 格地圖與資金也相同。
...
變成「查表」，兩個一直被掩蓋的真錯誤當天就露出來了：需求閥門的計算該用
32 位元浮點與整數除法（整座城市的地圖差異因此從 8 格變成 0 格），以及對拍
腳手架自己多推了四步亂數（症狀極隱蔽：抽樣次數照樣對得上，只有吃到的
亂數值整整晚四個）。細節見
