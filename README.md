...
| 資料格式 | 9 | 六種格式（`.PGF`／`.PPF`／`.PSN`／`.PTF`／`.PSF`／`.cty`）；24 個圖形檔全解，連庫表外那塊（地圖縮圖 ＋ 自帶字型）都解了；32 個城市檔逐位元組 round-trip | 遮罩庫（旗標 `0x0100`）怎麼與前一庫配對繪製還沒解；兩份 `SOUNDDAT.PSF` 不知道哪一份會被讀 |
...
| 可玩完成度 | 8 | 上面三項，加上**與 DOS 原版的抽樣對拍**（[`docs/re/18-dos-parity.md`](docs/re/18-dos-parity.md)）與**自動玩家**（[`internal/autoplay`](internal/autoplay)）：只用玩家在畫面上做得到的操作（`ApplyTool` 與稅率），八個劇本 40 次跑過 26 次（漢堡、伯恩、東京、波士頓五顆種子全過；底特律與里約各 3/5）；不出手只過 5 次 | **達斯維利與舊金山五顆種子全掛**——前者是 bootstrap 問題（每年能蓋的格數受限於稅收、稅收又受限於人口，複利起不來），後者只有五年而地震把路網打斷了。而且**還是沒有人用手玩通過**：自動玩家證明的是「規則允許贏」，不是「介面好用」 |
