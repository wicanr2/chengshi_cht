...
連同當年那幾本說明書一起保存下來。自備一份合法的原版磁片，那個位子就是你的。
...
第四個「地形編修程式」是 remake 加的），繁體中文為
「建立新城市／載入城市／選擇劇本／地形編修程式」。標誌、原版那三個按鈕的框線與
...
![地形編修程式](docs/images/terrain-editor.png)

1990 年隨磁片附的《SimCity Terrain Editor》。原版是獨立程式，remake 把整支做進來：
三個選單（系統／地形／變數）、左邊六格工具盤（開濶地、綠地、河流、航道四個畫筆，
加上均佈與回手）、編輯視窗、右邊的地圖視窗。**它用的是遊戲本體那一套視窗系統**，
座標與遊戲完全相同——這是把原版跑起來逐像素量出來的。
畫筆寫進地圖的值來自遊戲與編輯器共用的工具描述表，回手是五千格的環加四份全圖快照。
出口照原版：編輯器只存檔，回遊戲之後自己讀那個檔。

**畫面上這些字不是本專案翻的。** 軟體世界 1990 年另外出過一張資料卡
（編號 220，NT$150，書腰寫「須與珍藏版 29 配合使用」），裡面就是這支編輯器的
中文說明書：`DIRT` 叫**開濶地**、`FILL` 叫**均佈**、`UNDO` 叫**回手**、
程式本身叫**地形編修程式**。整組照抄那一本，逐條對照在
[`docs/manual-cht/naming-crosswalk.md`](docs/manual-cht/naming-crosswalk.md) 第二節。

![地形編修程式的地形選單](docs/images/terrain-menu.png)

「地形」選單拉下來。清除非自然物、綠地平滑、水區平滑、產生島嶼地形——
七條命令的中文全部出自 220 那本說明書的第 20–22 頁，連 `Creat Island`
少一個 `e` 的原文拼字錯誤都照原樣記在轉錄裡，沒有代為訂正。
...
| 同一座城市換成中世紀資料片。**工具名稱跟著換**——發電廠叫「水車」、鐵路叫「馬車道」、體育館叫「馬術競技場」。那是原版六個資料片的設計，不是翻譯自由發揮；中文用字出自電腦休閒世界 022 那本說明書。 | 地圖視窗，犯罪率圖層。十一個圖層都在，用 1–9、0、`-` 切換。 |
...
![古代亞洲的工具盤與圖形名稱欄](docs/images/style-tools.png)

古代亞洲資料片，點下工具盤第五列的警察局那一格，視窗左下角的圖形名稱欄顯示
**衙門：$500**。同一格在中世紀是**警局**、在西部是**警長**、在未來歐洲是
**警防部**、在月球是**月球警署**——這些不是本專案想的詞，是 1990 年代
台灣代理商說明書上的字。

專案手上有**四本**當年的中文說明書，畫面上每一個名詞都對得回其中一本的某一頁：

| 畫面上的字 | 出自哪一本 | 哪一頁 |
|---|---|---|
| 悲情城市、明日之都、市鑰、達斯維利 | 軟體世界珍藏版 29 | p.13–14 |
| **大都會區**（`Megalopolis`）、大都會（`Metropolis`）| 軟體世界珍藏版 29 | p.1、p.18 |
| 推土機、鐵軌、全自動整地、預算與評估視窗的欄位 | 軟體世界珍藏版 29 | p.23–58 |
| **地形編修程式**、開濶地、綠地、航道、均佈、**回手**、地形變數、河流曲率 | 軟體世界 220《地形編修程式》 | p.13–22 |
| **衙門**、**廟宇**、**摔角場**、**港灣**、水井灌溉系統、水車灌溉系統 | 電腦休閒世界 022《古城風情系列》 | p.56 圖版 |
| **警局**、**馬術競技場**、城堡 ／ **警長**、**賽馬場**、**金礦區**、蒸汽泵 | 電腦休閒世界 022 | p.59、p.62 圖版 |
| **小鎮**、**首都**、村莊／城市（城市類別）| 電腦休閒世界 022 | p.46 |
| **警防部**、**雷射競技場**、**水翼船港**、**太空站**、核子融爐電廠 | 電腦休閒世界 024《回到未來系列》 | p.17–18 圖版 |
| **月球警署**、**自由港**、**太空梭站**、**引力波電廠** | 電腦休閒世界 024 | p.19 圖版 |
| 現代／古代亞洲／中世紀／西部拓荒／未來美國／未來歐洲／月球殖民地（圖形集對話框）| 電腦休閒世界 022 ／ 024 | p.55 ／ p.6 |

**四本不是四份獨立證據。** 電腦休閒世界 022 的第一部份與軟體世界珍藏版 29 是
**同一份中文稿**——選單、工具、視窗欄位、八個劇本的說明文逐句相同，連圖號
（「圖八 編輯視窗」）都對得上，而兩家是各自向 MAXIS 取得授權的不同代理商；
024 的 PART B 又是 220 那本的重印。血緣圖、逐項對照與每一條取捨的理由寫在
[`docs/manual-cht/naming-crosswalk.md`](docs/manual-cht/naming-crosswalk.md)。

**說明書比原文粗的地方照採，不偷偷改掉，但要記明落差。** 例如
`Sumo arena` 說明書寫「摔角場」（不是相撲）、`Rodeo` 寫「賽馬場」（不是牛仔競技）、
`Hovercraft Port` 寫「水翼船港」——**hovercraft 是氣墊船，水翼船是 hydrofoil，
那是 1990 年就譯錯的船種**。三處都照當年的字，落差逐條記在對照表裡。

反過來，有三處**刻意沒照說明書**：月球的兩座電廠（休 024 的圖版與正文自相矛盾，
取正文）、月球的體育館（照改會讓月球的字串退回基本檔，那正是
`TestStyleFilesDoNotInheritBaseWording` 在擋的漏改）、以及 `Query`
（休 022 作「質詢回答」，把它當成議會質詢，與「點一格看資訊」對不上）。
...
四種語言都涵蓋整套介面與遊戲內文字：狀態訊息、圖片訊息、八個劇本簡介、
查詢面板的程度詞、預算與評估視窗的欄位，以及**六個資料片各自的用詞**——
古代亞洲的發電廠是「水車」、中世紀的鐵路是「馬車道」，日文與简中一樣要換，
不能退回基本檔的字。有一支測試釘著這件事（`TestStyleFilesDoNotInheritBaseWording`
與它的日文版）：原文換了字而譯文沒換就會紅，因為那種漏改看不出來——檔案是齊的、
沒有空字串、畫面也正常。英文不進版控，執行時從玩家自己那份 `.PTF` 讀。
...
台灣當年為這款遊戲印過的中文說明書，這個專案手上有四本，全部逐頁轉錄：

| 說明書 | 出版 | 涵蓋 | 轉錄 |
|---|---|---|---|
| 珍藏版 29《模擬城市》| 軟體世界 | 原版操作手冊 ＋ 參考手冊（p.1–82）| [`docs/manual-cht/`](docs/manual-cht/) |
| 220《模擬城市 地形編修程式》| 軟體世界 | 地形編輯器（p.1–23）| [`sw220-terrain/`](docs/manual-cht/sw220-terrain/) |
| 022《模擬城市－古城風情系列》| 電腦休閒世界（MAXIS 授權）| 原版操作 ＋ 古亞／中世紀／西部（p.1–64）| [`chw022-ancient/`](docs/manual-cht/chw022-ancient/) |
| 024《模擬城市－回到未來系列》| 電腦休閒世界（MAXIS 授權）| 美國／歐洲／月球 ＋ 地形編修程式重印（p.1–40）| [`chw024-future/`](docs/manual-cht/chw024-future/) |

珍藏版 29 那本由骨灰集散地的「軟體世界說明書補完計劃」掃描保存，共 56 張跨頁。
它留下的不只是操作步驟，還有一整套 1990 年代初的台灣譯法：
...
`translations/glossary.md` 以說明書既有譯名優先，四本都沒收的詞才另譯，並在表上標明。

另外三本補上的，正好是珍藏版 29 沒涵蓋的三塊：**地形編輯器整組**（220）、
**六個資料片的建築名與六種圖形集的名字**（休 022／024）、
**城市類別的六個級距**（休 022 p.46）。那三塊原本標「說明書未收，本專案新譯」，
現在整組換成當年的字——畫面上的字對得回哪一本哪一頁，見上面
[畫面上的字都有出處](#provenance)。
...
當年這是**四個分開賣的產品**。要湊齊得買三片以上的磁片，各自跑 `INSTALL.EXE`
把檔案倒進遊戲目錄；地形編輯器還得離開遊戲另外啟動。

| 產品 | 發行 | 內容 |
|---|---|---|
| SimCity | 1989，Brøderbund／Maxis | 基本外觀一套 |
| **Graphics Set #1（古城風情）** | Brøderbund | 古代亞洲、中世紀、西部拓荒 |
| **Graphics Set 2 — Future Cities（回到未來）** | Maxis | 未來美國、未來歐洲、月球殖民地 |
| **SimCity Terrain Editor** | Maxis；台灣由**軟體世界研究中心（高雄）**1990 年重新打包 | 獨立執行檔 `TERRAIN.EXE` |

⚠ 兩片資料片的年份（1990／1991）只有打包者的 `file_id.diz` 可查，**那是二手**；
磁片裡的檔案時間戳混著 1990 到 1993，與它對不起來。可確定的是**發行商換過手**：
Set #1 掛 Brøderbund，Set 2 掛 Maxis。

現在它們是一支程式。系統選單切得動七種外觀（現代 ＋ 六個資料片），
招牌上的第四顆按鈕直接進地形編修程式，全部繁體中文。

**Graphics Set #1（古城風情）：古代亞洲、中世紀、西部拓荒**

![古城風情三個風格](docs/images/styles-ancient.png)

**Graphics Set 2 — Future Cities（回到未來）：未來美國、未來歐洲、月球殖民地**

![回到未來三個風格](docs/images/styles-future.png)

六張都是**同一座城市、同一個鏡頭**（底特律 1972，格子 30,30），只換資料片——
所以差別全在美術。連左邊的工具盤圖示都跟著換，而且工具**名稱**也換：
中世紀的鐵路叫「馬車道」、發電廠叫「水車」；未來美國的道路叫「輸送管」、
鐵路叫「磁浮軌道」；月球殖民地的鐵路叫「軌道」。那是原版的設計，
每個圖形集自帶一份 `*_MSG.PTF`。

**SimCity Terrain Editor：原本是另一支程式**

![地形編輯器的參數對話框](docs/images/terrain-params.png)

原版 `TERRAIN.EXE` 的「地形參數」對話框——樹木數量、湖泊數量、河流彎曲三個
百分比，按「開始」產生地形。版面是把原版跑起來**逐像素量**出來的，六個增減鍵的
欄號與四個列號都對得上。三個百分比就是 `s_gen.c` 的 `TreeLevel`／`LakeLevel`／
`CurveLevel` 本身。編輯器的主畫面在[上面](#shots)。

### 哪些是原版就有的、哪些是這個專案加的

分清楚這件事比宣稱功能多重要。**原版本來就會的不算貢獻**：

| 功能 | 誰的 |
|---|---|
| 遊戲內切換圖形集 | **原版**——訊息檔第 17 段第 3 列就是「讀取圖形集」 |
| 換資料片連文字一起換（工具名、訊息、劇本簡介）| **原版**——每個圖形集自帶一份 `*_MSG.PTF` |
| **地形編輯器在遊戲裡** | remake。原版是要離開遊戲另外啟動的獨立程式 |
| **遊戲內切換顯示模式** | remake。原版要跑 `SETTINGS.EXE` 改 `SIMCITY.CFG` 再重開（資料片自己的 `README` 也是這麼教的：「use the Settings program」）|
| **繁體中文** | remake。原版從來沒有中文版（見[上文](#no-chinese)）|
| 縮小、多語、背景音樂 | remake |

所以這一節的「整合」指的是**六個資料片的美術與文字、地形編輯器、六種顯示模式
在同一支程式裡都到得了**，而且全部有繁體中文——地形編輯器尤其：它原本是一支
英文的獨立程式，這是它第一次有中文介面。六個資料片的譯文合計 695 條，
工具名跟著資料片換（中世紀的發電廠叫「水車」），那是原版的設計，翻譯照做。

### ⚠ 整合的是程式，不是素材

六個圖形集的美術與文字仍然來自玩家自己那幾片磁片，本專案不散布任何一片的內容。

而且**沒有任何一份原版發行同時具備全部素材**：

- DOS 1.10 有六組資料片美術與 mcga，但**缺 CGA 與 Tandy**。
- DOS 1.03 有六種顯示模式的基本外觀，但**缺 mcga，也沒有資料片**。
- 兩片資料片自己帶滿六個模式的美術，但**沒有基本外觀**。

要六種顯示模式 × 七組外觀全部齊全，得同時擁有 1.03 與兩片資料片。缺哪一塊，
遊戲就會在那個組合上明確報錯，不會靜靜退回別的模式。
...
| 中文化與多語 | 9.8 | 遊戲內 695 條全譯（繁中）；說明書 p.1–82 逐頁轉錄 ＋ 參考附表。**語言資料是 TSV，一列一筆四種語言並排**，遊戲內可切繁中／简中／日文／English（`-lang`，或系統選單）。**繁中、简中、日文三種在基本訊息檔都是全譯**（226／235，其餘 9 筆原文本來就不是文字），**六個資料片各自的用詞也逐條換過**（古代亞洲的發電廠是「水車」不是「原子力発電所」）——這一項有測試釘著（`TestStyleFilesDoNotInheritBaseWording` 與它的 `Ja` 版）：原文換了字而譯文還是基本檔的就會紅。畫面上所有玩家看得到的字都走 `ui.tsv`，`TestNoHardcodedChineseOnScreen` 擋掉直接送中文字面上畫面的寫法，`TestPictureMessagesFitTheBriefDialog` 驗三種語言的圖片訊息都放得進對話框 | 简中是繁中的**字面**轉換，不是用語在地化；日文是**本專案新譯**，沒有原版可以對照，也**還沒有母語者校過**；英文取自玩家自備的原版 `.PTF`，沒有原版就沒有英文 |
...
**這一版只有一列動了：中文化與多語 9.5 → 9.8。** 動的理由是那一列的扣分項少了一條
——先前日文只做到標籤，遊戲內的敘事文字（狀態訊息、圖片訊息、八個劇本簡介）走退路
顯示繁體中文，現在補完了，六個資料片各自的用詞也跟著換。剩下的兩條還在：
简中是字面轉換，日文沒有母語者校過。所以是 9.8 不是 10。

**其餘七列沒有動，即使這一版動的東西不少。** 分數綁的是「還剩什麼沒解」，
不是「做了多少事」：呈現層這一輪修掉二十幾處沒走文字層的畫面字串、把劇本簡介的
按鈕寬度改成跟著字走、補了三道測試，但那一列列的三個未解（視窗預設位置、
原版怎麼蓋核能廠、數值圖層第五級門檻）一個都沒少，所以還是 9.5。
資料格式同理：`.PGF` 第 8、9 庫與 `SOUNDDAT` 該讀哪一份沒解開就上不了 10。
...
tools/package_all.sh v.1.5.1-20260904       # 重建 dist-all/<版本> 的 full／release／promo
tools/verify_package_all.sh v.1.5.1-20260904 # 另驗 AppImage 招牌入口、滑鼠選單與音訊
...
| [`docs/manual-cht/`](docs/manual-cht/) | **四本**中文說明書的逐頁轉錄，加上 [`naming-crosswalk.md`](docs/manual-cht/naming-crosswalk.md)：四本的血緣、逐項譯名對照與取捨理由 |
