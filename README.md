...
| 可玩完成度 | 8 | 上面三項，加上**與 DOS 原版的抽樣對拍**（[`docs/re/18-dos-parity.md`](docs/re/18-dos-parity.md)）：八個劇本，載入層 96.4%–99.4% 逐格相同；讓原版真的跑一段（劇本 1 跑 21 年）之後，住宅／商業／工業／道路／鐵軌／電線／樹／水的格數逐項相同或差不到 0.2% | **汙染均值比原版高 15%–60%**，三個劇本方向一致；下游的地價、商業需求、人口與資金跟著低（資金差三成）。原因還沒拆開，候選與下一步寫在筆記裡。**也還是沒有人真的從頭玩通任何一個劇本**——不出手跑只有 2/8 過 |
...
扣分集中在三件事——**沒有聲音**、**汙染算得比原版高、整條經濟鏈跟著低三成**，
