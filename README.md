...
| 音效 | 3 | 容器格式解開（九份檔各八段 4 位元 PCM）；執行檔脫了殼，符號表解出來了——音效的三支常式都在模組 `0x23`（[`docs/re/16-dos-oracle.md`](docs/re/16-dos-oracle.md) §5）| **還是沒有接進遊戲。** 發聲常式找到了（映像 `0xC9EB`），三條路分別是 DAC、Tandy 音效 BIOS（`INT 1Ah AH=83h`——DOSBox-X 不提供，所以 Tandy 在模擬器裡必然無聲）與 PC 喇叭；但「哪一段是哪個事件」在呼叫端，而那是 far call、節區值載入時才填，靜態搜不到 |
