...
- **沒有聲音**：`.PSF` 的容器格式解得開（八段 4 位元 PCM），但那八段**只走
  外接 DAC**——內建喇叭放的是程式自己合成的嗶聲，換一份音效檔錄同一組動作，
  聲音逐取樣相同。而手上的環境兩條 DAC 路都走不通：Covox Sound Master 沒有
  模擬器支援，Tandy 需要這份副本缺少的圖形檔。在對出「哪一段是哪個事件」之前
  不接進遊戲——播錯聲音等於把一條沒有證據的斷言寫進產品。
...
| 音效 | 4 | 容器格式解開（九份檔各八段 4 位元 PCM）；執行檔脫了殼，符號表解出來了——音效的三支常式都在模組 `0x23`（[`docs/re/16-dos-oracle.md`](docs/re/16-dos-oracle.md) §5）| **還是沒有接進遊戲。** 發聲常式找到了（映像 `0xC9EB`），三條路分別是 DAC、Tandy 音效 BIOS（`INT 1Ah AH=83h`——DOSBox-X 不提供，所以 Tandy 在模擬器裡必然無聲）與 PC 喇叭；`PlaySound(n)` 也找到了（`0xCF51`，n ＝ 0–7 的段編號，十個呼叫點），八段裡認出三段（7 ＝ 工具失敗、5／6 ＝ 工具成功）；**其餘五段與取樣率仍未解**，而且手上沒有能放出 DAC 的模擬器可以驗證 |
...
扣分集中在兩件事——**沒有聲音**（卡在證據，見上一節），以及**沒有人真的用手
把一座城市或一個劇本玩完**（自動玩家證明了規則允許贏，那不是同一件事）。
後者需要一個人坐下來玩，不是工具能代勞的。
