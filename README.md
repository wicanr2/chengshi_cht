...
![地形編輯器的參數對話框](docs/images/terrain-editor.png)

1990 年隨磁片附的《SimCity Terrain Editor》的參數對話框。三個百分比就是
`s_gen.c` 的 `TreeLevel`／`LakeLevel`／`CurveLevel`，按「開始」產生地形再進遊戲。
版面（六個增減鍵的欄、四個列、按鈕尺寸、配色）是把原版跑起來逐像素量出來的，
與反組譯推的欄列互相印證。原版還有六個畫筆工具與三個選單，那些還沒做。
...
本專案補的台北台中台南、**1990 年那支地形編輯器的參數對話框**、從一片空地蓋起一座城市、
...
| 城市外觀 | 基本 ＋ 六個資料片風格，四種顯示模式的圖形檔都解得開；**六個風格都與原版做過畫面級對拍** |
| 地形編輯器 | 1990 年隨磁片附的 `TERRAIN.EXE` 的**參數對話框**（樹木／湖泊／河流彎曲三個百分比 ＋ 開始／取消），版面照原版量測重製；原版那六個畫筆工具與三個選單還沒做 |
...
`.PGF` 第 8、9 庫、`SOUNDDAT` 該讀哪一份、CGA 的四色盤，三件都還在，資料格式就上不了 10。
