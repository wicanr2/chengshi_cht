...
- **沒有聲音**：`.PSF` 的容器格式解得開（八段 4 位元 PCM），**八段各是什麼事件
  也解出來了**——脫殼反組譯 `PlaySound(n)` 的十一個呼叫點，逐支比對 Micropolis
  的對應函式：0 交通壅塞、1 爆炸、2 怪獸、3 警笛、4 船笛、5／6 工具成功、
  7 工具失敗（[`docs/re/16-dos-oracle.md`](docs/re/16-dos-oracle.md) §五之四）。
  **剩下的是取樣率。** 長度比量得出 5300–5450 Hz，但那是強證據不是直讀；
  常數在音效卡驅動的初始化那一段，還沒讀到。在它定值之前不接進遊戲——
  用錯速率放出來的聲音，音高與長度都是錯的。
  另外**用耳朵驗收這件事目前做不到**：那八段只走外接 DAC，而 Tandy 走的是
...
| 資料格式 | 9 | 六種格式（`.PGF`／`.PPF`／`.PSN`／`.PTF`／`.PSF`／`.cty`）；24 個圖形檔全解；32 個城市檔逐位元組 round-trip | `.PGF` 第 0 庫後面那塊共用資料（CEGA 11 523 位元組）沒有逐張分界；兩份 `SOUNDDAT.PSF` 不知道哪一份會被讀 |
| 呈現層與操作 | 8 | 原版《參考附表》與訊息檔第 17–21 段列出的按鍵與選單項，逐項對照（[`docs/spec/controls.md`](docs/spec/controls.md)）| 四個選單項沒接（關於本遊戲、印表、以檔名儲存、視窗位置／大小）；原版五段速度對到規則層四段；沒有下拉選單，兩個選單做成視窗 |
| 中文化 | 9.5 | 遊戲內 695 條全譯；說明書 p.1–82 逐頁轉錄 ＋ 參考附表 | 六個圖形集的風格名說明書沒收，是本專案新譯；密碼表**刻意**不轉錄 |
| 音效 | 6 | 容器格式解開（九份檔各八段 4 位元 PCM）；**八段的事件對應全部解出**——`PlaySound(n)` 的十一個呼叫點逐支反組譯，與 Micropolis 對應函式比對到位移表、門檻值與訊息編號都一致（[`docs/re/16-dos-oracle.md`](docs/re/16-dos-oracle.md) §五之四）| **還是沒有接進遊戲，卡的只剩取樣率。** 三段獨立的長度比指向 5300–5450 Hz（強證據），常數在音效卡驅動的初始化那一段，還沒讀到；已排除 PIT 中斷餵樣本（整份執行檔沒有一次 `out 40h`，那是一張 DMA 驅動的卡）。而且手上沒有能放出 DAC 的模擬器，聽不到原版的聲音來對 |
| 可玩完成度 | 8 | 上面三項，加上**與 DOS 原版的抽樣對拍**（[`docs/re/18-dos-parity.md`](docs/re/18-dos-parity.md)）與**自動玩家**（[`internal/autoplay`](internal/autoplay)）：只用玩家在畫面上做得到的操作（`ApplyTool` 與稅率），八個劇本平均過五個（漢堡、伯恩、東京、波士頓五顆種子全過；底特律 3/5、里約 2/5）；不出手只過一個 | **達斯維利與舊金山五顆種子全掛**——前者是 bootstrap 問題（三十年存款都在 3 000–6 000 打轉），後者只有五年而地震把路網打斷了。而且**還是沒有人用手玩通過**：自動玩家證明的是「規則允許贏」，不是「介面好用」 |
| 跨平台發行 | 8 | Linux／Windows／macOS universal 三個包，`tools/verify_release.sh` 驗包本身 | macOS 版**沒有實機試玩**（Linux 上執行不了 macOS binary），靜態驗收全過只代表不會因結構問題開不起來；沒有 Android 包 |

**整體 8 / 10。** 權重不平均：規則層與中文化是這個專案的目的，兩項都接近滿分；
扣分集中在兩件事——**沒有聲音**（八段是什麼已經解出來，卡在取樣率，見上一節），
