...
| 面向 | 自評 | 量法與分母 | 分是扣在哪 |
|---|:-:|---|---|
| 模擬規則層 | 9.5 | 四份逐 frame 對拍各 8000 個 frame，東京那份 955 206 次抽樣，判準含精靈全欄位與整張地圖雜湊 | 兩處**刻意**偏離原版（`oFireZone` 補了邊界檢查、載入不重設亂數種子），都寫在筆記裡；沒有跨越八千個 frame 之外的長時間對拍 |
...
| 音效 | 2 | 容器格式解開（九份檔各八段 4 位元 PCM），事件對應未解 | **沒有接進遊戲。** 那八段只走外接 DAC，而手上兩條 DAC 路都走不通；在對出「哪一段是哪個事件」之前不接 |
| 可玩完成度 | 8 | 上面三項，加上**與 DOS 原版的抽樣對拍**（[`docs/re/18-dos-parity.md`](docs/re/18-dos-parity.md)）：八個劇本，載入層 96.4%–99.4% 逐格相同；讓原版真的跑一段（劇本 1 跑 21 年）之後，住宅／商業／工業／道路／鐵軌／電線／樹／水的格數逐項相同或差不到 0.2% | 與 DOS 原版有一處**已確認的規則差異**：DOS 算的汙染比 Micropolis 低 8%–43%（16 份存檔三方對質，remake 與 Micropolis 全部落在 ±7 以內），下游的地價、需求、人口與資金跟著差三成。本專案照 Micropolis，不往 DOS 調。**也還是沒有人真的從頭玩通任何一個劇本**——不出手跑只有 2/8 過 |
...
扣分集中在三件事——**沒有聲音**、**與 DOS 版有一處已確認但還沒拆開的規則差異**，
以及**沒有人真的把一座城市或一個劇本玩完**。第一件卡在證據（見上一節），
第二件有明確的下一步（[`docs/re/18-dos-parity.md`](docs/re/18-dos-parity.md) §六），
第三件是還沒做的工。
