...
| 問題 | 現況 | 要怎麼定案 |
|---|---|---|
| 體育館與海港的造價 | 說明書寫體育館 $3000、海港 $5000；Micropolis 的 `CostOf[]` 是體育館 5000、海港 3000，**剛好對調**。規則層目前以 Micropolis 為準（可執行、可對拍）。 | 反組譯 `SIMCITY.EXE` 的成本表 |
...
19. **166 條遊戲訊息的翻譯**（`translations/messages/`，以序號為鍵）。
20. **呈現層**（`internal/ui`，Ebiten）：圖塊渲染、工具列、視窗、輸入。
21. 正常玩家路徑試玩驗收、發行包。
