...
逐欄位的短版 54/400，是現在的前線；分岔只在精靈那一側。
...
| 精靈加進串列的位置 | 原始碼的 `NewSprite` 是**前插**，但逐 frame 逐欄位對拍量出來是**尾端**（前插 2/400、尾端 54/400）。實作照量出來的走 | 讀 `sim->sprite` 的其他使用點，或在 oracle 裡直接把串列的實體順序（含死掉的節點）倒出來 |
| Tokyo 短版第 54 個 frame，原版在精靈那一側多抽一次 | 怪獸之前還有一隻抽了我們沒抽的那一次；火車那時 `Cycle & 3 == 3`、船的計數也沒歸零 | 在 oracle 裡逐**隻**記抽樣次數（現在只記到「精靈那一側」整體）|
...
13. ~~精靈系統~~ **完成**：`docs/re/13-sprites.md`，
    ...
    十八個欄位**的 `TestSpriteParity`（54/400）。分岔只在精靈那一側
