...
逐欄位的短版 46/400，是現在的前線；分岔只在精靈那一側，卡在精靈加進
串列的位置。
...
| 精靈加進串列的位置 | 原始碼的 `NewSprite` 是**前插**，實測卻既不是最前也不是最後（怪獸生出來時原版排第三、我們排第四）。合理解釋是同型別的舊節點會被**原地重用**，所以順序取決於「這個型別第一次被建立是什麼時候」，而那段歷史包含開機那座隨機城市。實作先照量得最遠的（尾端）走 | 在 oracle 裡把串列的**實體順序（含死掉的節點）**倒出來 |
...
13. ~~精靈系統~~ **完成**：`docs/re/13-sprites.md`，
    ...
    十八個欄位**的 `TestSpriteParity`（46/400）。分岔只在精靈那一側
