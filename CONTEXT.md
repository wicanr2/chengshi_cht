...
**2026-08-30 大改：操作介面換成原版 DOS 的版面**，量測與現況在
...
City Form 視窗、三個資料視窗（位置大小與內部版面）、查詢面板、
視窗疊放與搬動、`Ctrl-C／H／E／R`、五段速度、工具盤十四格 ＋ 發電廠副選單。
試玩腳本 `tools/playtest.sh` 在新版面上全部通過。
...
| 問題 | 現況 | 要怎麼定案 |
|---|---|---|
| 兩份 `SOUNDDAT.PSF` 哪一份被讀 | 只差第 2 段（怪獸）：根目錄 7500 取樣、`DATA/` 10752（與 1991 的 `.V4` 一致）。remake 取 `DATA/`，依據是執行檔字串表的相鄰性——**推論不是實證** | **只剩反組譯開檔那一段。** DOSBox 追檔案開啟這條路已排除：模擬器裡 Covox 與 Tandy 兩條發聲路徑都活不起來（畫面明寫 `Sound Master not found. Using internal speaker`），檔案從頭到尾沒被開啟，弄壞它畫面零差異；而弄壞 `MESSAGE.PTF` 的正對照差兩萬像素，所以不是方法失靈。見 [`docs/re/16-dos-oracle.md`](docs/re/16-dos-oracle.md) §八 |
...
| `.PGF` 第 9 庫的用途 | 12 張 16×16 單平面圖，是「16 像素格子切成四個半格」之後的全部一格／相鄰兩格／三格組合。七個 CEGA 檔位元相同，四種顯示模式下都是 16×16（地圖圖塊在 MCGA 與低解析 EGA 是 8×8），所以它不在地圖格網上 | 找出繪製它的呼叫點。原版截圖逐點搜過只有零星的純色方塊巧合命中 |
| `.PGF` 第 8 庫在什麼時候被畫出來 | 基本檔是 107×20 的一行日文「ウルトラ警備隊」（四種顯示模式各自獨立編碼、解出同一串字），風格包裡是空的 | 同上。是不是彩蛋目前只到假說 |
