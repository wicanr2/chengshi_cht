...
**六種顯示模式在遊戲裡切得動了，面板也跟著換。** `-mode` 或系統選單第 16 列，
六種都能載入（EGA 高／低解析彩色、Tandy 彩色、VGA 256 色、單色、CGA 單色），
換的是**全部美術**——地圖圖塊、精靈、City Form 縮圖，加上工具盤、需求指標、
統計圖按鈕、圖層圖示與色階。⚠ **不換的是版面**：remake 只有一套 640×350 的
座標。介面美術一換，格線就得跟著換（六個模式的工具盤 57×182 到 55×120 都有，
