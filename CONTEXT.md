...
`v.1.4.1-20260904`，舊 `20260901` Release 與 tag 已依使用者決定刪除，不得復用。
...
**六種顯示模式在遊戲裡切得動了，面板與配色都跟著換。** `-mode` 或系統選單
第 16 列，六種都能載入（EGA 高／低解析彩色、Tandy 彩色、VGA 256 色、單色、
CGA 單色）。換的是**全部美術加配色**——地圖圖塊、精靈、City Form 縮圖，
工具盤、需求指標、統計圖按鈕、圖層圖示與色階，以及選單列、視窗框、標題列、
資金帶、工具帶、對話框、選取框的顏色。⚠ **不換的是版面**：remake 只有一套
640×350 的座標。
介面美術一換格線就得跟著換（六個模式的工具盤 57×182 到 55×120 都有，
圖層圖示連排法都不同），六組量測值在 `internal/ui/uigeom.go`；
配色分兩套（`internal/ui/chrome.go`）——**原版的單色與 CGA 整張畫面
只有黑白兩色**，這是實測的（`tools/chromesample` 數原版截圖）。
理由與量法在 [`docs/spec/ui-layout.md`](docs/spec/ui-layout.md) 的「顯示模式」一節。
...
- **四種語言的來歷不一樣**（`translations/README.md`）：繁中是翻譯、
  简中是繁中的**字面**轉換（不是用語在地化）、日文是本專案新譯、
  英文是執行時從玩家自己那份 `.PTF` 讀的，沒有原版就沒有英文。
  繁中、简中、日文三種在基本檔都是全譯，六個資料片各自的用詞也逐條換過；
  漏改由 `TestStyleFilesDoNotInheritBaseWording` 與它的 `Ja` 版釘著。
...
40. ~~六種顯示模式可以在遊戲裡切換~~ **完成**（使用者定案 2026-09-04）：
    命令列 `-mode cega／sega／tdy／mcga／mono／cga`，或系統選單第 16 列
    「顯示模式」。設定會記進 `settings.json` 的 `display_mode`。
    ⚠ **換的是全部美術，不換版面**——地圖圖塊、精靈、City Form 縮圖與
    介面美術（工具盤、需求指標、統計圖按鈕、圖層圖示、色階）都換成該模式
    自己的，換不掉的是那一套 640×350 的座標。介面美術一換格線就得跟著換：
    六個模式的工具盤是 57×182／56×123／56×119／56×182／55×120，
    **圖層圖示連排法都不同**（CEGA 與 MONO 直的一欄九列、sega／tdy／mcga
    兩欄五列、**CGA 橫的九欄一列**），拿 CEGA 的格線去點 tdy 的美術會
    點到隔壁那一格。六組量測值在 `internal/ui/uigeom.go`。
    **配色也跟著換**：原版實跑六個模式各截一張，用 `tools/chromesample`
    數顏色——CEGA 與 Tandy 是標準 EGA 十六色，mcga 的介面逐項與 EGA 相同，
    而**單色 VGA 與 CGA Mono 整張畫面只有 `#000000` 與 `#ffffff` 兩色**。
    所以配色分兩套（`internal/ui/chrome.go`），彩色那一套一個值都沒改。
    單色那一套的選取框改畫**網點外框**——原版就是這樣做的，只有兩色時
    實線框會與按鈕自己的外框混在一起。
    招牌與劇本選單只有單色換得過去（其餘模式的是 320×200，換上去
    三顆按鈕的點擊區會整個錯位）。
    結果是「用 Tandy／CGA 的美術玩 EGA 版面」，**原版沒有這種畫面**，
    是 remake 自訂的組合（`docs/spec/ui-layout.md` 的「顯示模式」一節）。
    做這件事的路上修掉三個沉默的解碼錯：**Tandy 的 16 色是封裝式不是 EGA
    平面式**（位元組數一樣，長度檢查抓不到，畫出來像檔案壞了）、
    **Tandy 的 City Form 縮圖也是封裝式**（`per % planes` 過不了直接回 nil，
    縮圖整片退回純色方塊而畫面照樣畫得出來）、**CGA 基本檔的圖塊是 16×8**
    （之前寫 16×16 的 8bpp，`-style base -mode cga` 一開就錯）。
    另外 CGA Mono 的美術在 remake 要**縱向拉兩倍**（它的螢幕是 640×200，
    像素是扁的），不拉的話地圖區變成橫紋，而那看起來像解碼錯不像版面錯。
...
44. ~~推廣影片重拍，字卡三語同步~~ **完成**：文字在
    `translations/promo_cards.tsv`，一張卡三行（繁中白／日文青／英文綠）；
    新增語言視窗與日英兩則劇本簡介的實機段落。`promocard` 加了寬度與高度
    兩道閘門，塞不下就失敗而不是靜靜切掉。76.6 秒、29 段。
    隨 `v.1.4.1-20260904` 交付。

43. ~~交付 `v.1.4.0-20260904`~~ **完成**：內容是日文補完、畫面字串全部走 i18n，
    以及三份 README 各一套自己語言的截圖。

42. ~~日文補完，三份 README 各一套自己語言的截圖~~ **完成**：
    訊息檔的日文從 155/235 補到 226/235（剩下 9 筆原文就不是文字），
    六個資料片各補「用詞和基本檔不同」的 465 筆；`ui.tsv` 從 109 筆到 179 筆，
    `internal/ui` 裡寫死中文的畫面字串（劇本簡介按鈕、資金帶、預算欄標題、
    查詢程度詞、圖形集與顯示模式名、兩個關於頁）全部改成查鍵。
    漏改由 `TestStyleFilesDoNotInheritBaseWordingJa` 釘著，
