...
### City Form 的圖層圖示只有九格，圖層有十一個

原版 City Form 左緣的圖示欄是**九格**（量自 `workplace/dosbox/w3-01-maps.png`：
y 44 起、一格 25 像素、九格到 268），而地圖圖層有十一個（訊息檔第 10／11 段）。
最後那一格印著 `F` 與 `P`，看起來一格管兩個圖層。哪一格對到哪幾個圖層、
怎麼在同一格的兩個圖層之間切換，**還沒解**。

remake 現在把黃色選取框畫在前九格，第十、十一個圖層選中時不畫框。
下一個入口是原版對那一格連點兩次的行為。
