...
**2026-08-30 大改：操作介面換成原版 DOS 的版面**（選單列 ＋ 浮動視窗 ＋
圖示工具盤），量測與現況在 [`docs/spec/ui-layout.md`](docs/spec/ui-layout.md)。
畫布 1920×1050 ＝ 原版 640×350 × 3。介面美術直接用原版 `.PGF` 的庫 2／3／4／5／6／7。
...
- **音效**：八段的事件對應解開了（0 交通壅塞、1 爆炸、2 怪獸、3 警笛、
  ...
  **取樣率是暫代值 5400 Hz**：長度比量出的區間是 5300–5450（強證據，§五之五），
  沒有從程式碼直讀。下一個入口是音效卡驅動命令 `AH=04h/05h` 那個 16 位元參數
  （遊戲設 20）的單位。
...
| `.PSF` 八段的取樣率 | 三段獨立的長度比（段 0／1／4 對 X11 的 `traffic`／`expl-hi`／`honk-low`）落在 0.673–0.678，指向 5300–5450 Hz。初始化找到了（映像 `0xCC20`）：遊戲只設腳位、音量、和**一個值為 20 的 16 位元參數**（命令 `AH=04h/05h`）——那就是速率參數，缺的是它的單位 | 要 Covox Sound Master 的程式設計文件，或一個會模擬它的環境。已排除 PIT（整份執行檔沒有 `out 40h`）|
...
25. ~~聲音~~ **已接**（`docs/spec/sound.md`、`internal/audio`、
    ...
    **剩下的是取樣率的定值**：程式取 5400 Hz，區間是 5300–5450（強證據），
    規格裡標成暫代。下一個入口是驅動命令 `AH=04h/05h` 那個 16 位元參數
