...
**六種顯示模式在遊戲裡切得動了。** `-mode` 或系統選單第 16 列，六種都能載入
（EGA 高／低解析彩色、Tandy 彩色、VGA 256 色、單色、CGA 單色）。
⚠ 換的是**地圖美術不是版面**——remake 只有一套 640×350 的 EGA 高解析版面，
介面美術一律留 CEGA 的，理由與已知後果在
...
40. ~~六種顯示模式可以在遊戲裡切換~~ **完成**（使用者定案 2026-09-04）：
    ...
    ⚠ **換的是地圖美術，不是版面**——remake 只有一套 640×350 的 EGA 高解析
    版面，介面美術（工具盤、需求指標、圖層圖示、色階）一律留 CEGA 的，
    因為版面常數與**點擊區**都是照 CEGA 的美術量的。結果是「用 Tandy／CGA
    的地圖美術玩 EGA 版面」，**原版沒有這種畫面**，是 remake 自訂的組合
    （`docs/spec/ui-layout.md` 的「顯示模式」一節）。
